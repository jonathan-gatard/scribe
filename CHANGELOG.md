...
- **Every Home Assistant start scanned the whole history**: the initial "states written" counter ran `SELECT count(*)` against the `states` view, which aggregates every row across every chunk and decompresses each one. On a year-old install that is tens of millions of rows — measured at 90 million and all 103 chunks on the author's database — at every restart. The counter now reads TimescaleDB's planner statistics via `approximate_row_count()` — 21 ms against a full scan — but only past a million rows: below that, counting is cheap and the exact figure is kept. The exact count also remains the fallback when the statistics are absent (a fresh install, or a bulk import before autovacuum) or the extension is not installed.
