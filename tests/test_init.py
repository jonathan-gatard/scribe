...
async def test_statistics_setup(hass, mock_config_entry):
    ...
    with (
        ...
        patch(
            "custom_components.scribe.coordinator.ScribeDataUpdateCoordinator"
        ) as mock_coord_cls,
    ):
        mock_writer = mock_writer_cls.return_value
        mock_writer.start = AsyncMock()
        ...
        assert (
            hass.data[DOMAIN][mock_config_entry.entry_id]["chunk_coordinator"]
            == mock_coord
        )
    ...
    with patch("custom_components.scribe.ScribeWriter") as mock_writer_cls:
        ...
        hass.bus.async_fire(
            EVENT_STATE_CHANGED,
            {
                ...
                "new_state": State("sensor.excluded", "1"),
            },
        )
        await hass.async_block_till_done()
        ...
        hass.bus.async_fire(
            EVENT_STATE_CHANGED, {"entity_id": "sensor.included", "new_state": None}
        )
        await hass.async_block_till_done()
        mock_writer.enqueue.assert_not_called()
        ...
        hass.bus.async_fire(
            EVENT_STATE_CHANGED,
            {
                ...
                "new_state": State("sensor.included", "string"),
            },
        )
        await hass.async_block_till_done()
        mock_writer.enqueue.assert_called()
        ...
        hass.bus.async_fire(
            EVENT_STATE_CHANGED,
            {
                "entity_id": "sensor.included",
                "new_state": State(
                    "sensor.included", "1", {"excluded_attr": 1, "kept_attr": 2}
                ),
            },
        )
        await hass.async_block_till_done()
...
async def test_event_listener_no_record_states(hass, mock_config_entry):
    ...
    with patch("custom_components.scribe.ScribeWriter") as mock_writer_cls:
        ...
        hass.bus.async_fire(
            EVENT_STATE_CHANGED,
            {"entity_id": "sensor.test", "new_state": State("sensor.test", "1")},
        )
        await hass.async_block_till_done()
        mock_writer.enqueue.assert_not_called()
...
async def test_reload_entry(hass, mock_config_entry):
    ...
    with (
        patch(
            "custom_components.scribe.async_unload_entry", return_value=True
        ) as mock_unload,
        patch(
            "custom_components.scribe.async_setup_entry", return_value=True
        ) as mock_setup,
    ):
